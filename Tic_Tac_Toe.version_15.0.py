...
def make_move (player):
    ...
    if player =="X":
        ...
        prompt_begin = "Drugi"
        sign = "O"
    j=0 #czemu ta pętla jest w tym miejscu? bo program ma zapytać jednego gracza gdy jest jego kolejność jaką cyfrę wybiera aż 100razy
    while j<100:
        j+=1
...
if i == 4:
    ...
    for last_move in range(1,10):
        if last_move not in what_is_actually_in_field:
            print(last_move)
            make_move('X', last_move)
            break
