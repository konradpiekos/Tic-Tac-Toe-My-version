...
def ooo_xxx_move (current_player):
    if line_1[0] == current_player and line_1[1] == current_player and "3" not in what_is_actually_in_field:
        ...
        ooo_move_digit.append("5")
    elif line_1[1] == current_player and line_2[2] == current_player and "1" not in what_is_actually_in_field:
        ooo_move_digit.append("1")
    elif line_1[2] == current_player and line_2[1] == current_player and "7" not in what_is_actually_in_field:
        ooo_move_digit.append("7")
    elif line_1[2] == current_player and line_3[1] == current_player and "5" not in what_is_actually_in_field:
        ooo_move_digit.append("5")
    elif line_1[1] == current_player and line_2[0] == current_player and "3" not in what_is_actually_in_field:
        ooo_move_digit.append("3")
    else:
        None

def automated_move (current_player):
    ooo_xxx_move(current_player)
    random_move()

def random_move ():
    print ("znacznik 1")
    import random
    for random_move in range(1,100):  #TODO:  w teori może zawieść - zmienić żeby było stuprocenowo niezawodne
        random_move = random.choice(range(1,10))
        random_move = str(random_move)
        print(random_move)
        if random_move not in what_is_actually_in_field:
            applying_automated_move ("O",random_move)
            break
    # Napisać funkcję ktora zwroci wartosć pola ktore da wygraną lub zwroci None. Automated move powinna korzystac
    # z tej nowej funkcji, w ten sposob że wywołuje ją najpierw sprawdzajac czy sam moze wygrac atakiem a pozniej czy musi
    # sie bronic, a jezeli zaden z tych przypadkow nie zachodzi to robi randomowy ruch.

        #if zwrocona_wartosc is not None:
            #musimy się bronić - wykonaj odpowiedni ruch
...
def applying_automated_move_is_field_is_taken ():
    if computer_move in what_is_actually_in_field: # computer_move jest argumentem funkcji applying_automated_move
        return True
    else:
        return False
...
while i < 4:
   ...
   #Do przemyślenia w domu. Powinno się wykonywać najpierw automated_move od X pozniej od O
    automated_move("O")
    print (ooo_move_digit)# Ruch automatyczny musi zostać wykonany przed ruchem manualnym
    print_board()
